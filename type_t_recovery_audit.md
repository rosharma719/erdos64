...
| exact-two-attachment anchored A | either; `att(B_x)={x,X_x}` | gateways `{x,X_x}`; every bridge path starts `xY_x` | spectrum contains `2` and an unspecified admissible pair `L,L+delta`, `delta in {1,2}`; `3,4` are excluded | none | determine the larger admissible pair and its overlap; committed R2/S2 labels are unavailable |
