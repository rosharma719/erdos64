// t=3 analogue of the audited t4_intersection_filter.cpp, adapted by hand
// (the externally-supplied generic engine's source was not available --
// only its theorem writeup was). Same exact triangle-quotient interval
// theorem, same structure; changed: N=24 (quotient order for t=3 is
// 30-2*3=24), marks=3 (C(24,3)=2024 candidates/quotient instead of
// C(22,4)=7315), expand_marked builds 3 triangles (9 marked-expanded
// vertices) + 21 unmarked = 30 total, and the allowed_bits table is the
// independently-derived t=3 table (see derive_t3_table.py in this
// directory -- the derivation method was validated by exactly reproducing
// the given, audited t=4 table before being trusted here).
#include <bits/stdc++.h>
using namespace std;
...
static G expand_marked(const G&q,uint32_t S){vector<int>m,c;for(int v=0;v<q.n;++v)(((S>>v)&1u)?m:c).push_back(v);if(m.size()!=3)return{};int mid[32],cid[32],port[32][32];fill(begin(mid),end(mid),-1);fill(begin(cid),end(cid),-1);memset(port,-1,sizeof(port));for(int i=0;i<3;++i)mid[m[i]]=i;for(int i=0;i<(int)c.size();++i)cid[c[i]]=9+i;for(int i=0;i<3;++i){int k=0,u=m[i];uint32_t z=q.a[u];while(z){int v=__builtin_ctz(z);z&=z-1;port[u][v]=3*i+k++;}if(k!=3)return{};}G g{30,vector<uint32_t>(30),{}};for(int i=0;i<3;++i){int b=3*i;add_edge(g,b,b+1);add_edge(g,b+1,b+2);add_edge(g,b,b+2);}for(auto [u,v]:q.e){int U=((S>>u)&1u)?port[u][v]:cid[u],V=((S>>v)&1u)?port[v][u]:cid[v];if(!add_edge(g,U,V))return{};}return g;}
...
static uint8_t allowed_bits(int L){
 // bit e says |C cap S|=e is allowed, t=3. Independently derived (not
 // copy-pasted from an external source) by the same method used to
 // validate against the audited t=4 table: [L+e,L+2e] avoids {4,8,16},
 // plus L=3,e=0 struck (an unmarked quotient triangle is always an extra
 // triangle beyond the exactly-3 marked ones).
...
struct IntersectSolver{
 ...
 IntersectSolver(const G&Q,Stats&S):q(Q),st(S){
  cand.reserve(2024);for(int a=0;a<22;++a)for(int b=a+1;b<23;++b)for(int c=b+1;c<24;++c)cand.push_back((1u<<a)|(1u<<b)|(1u<<c));
...
int main(int argc,char**argv){string outdir=".";uint64_t maxg=ULLONG_MAX;bool all=false,verify=true;
 for(int i=1;i<argc;++i){string a=argv[i];if(a=="--outdir"&&i+1<argc)outdir=argv[++i];else if(a=="--max"&&i+1<argc)maxg=stoull(argv[++i]);else if(a=="--all")all=true;else if(a=="--no-literal-verify")verify=false;else{cerr<<"usage: t3_intersection_filter [--outdir D] [--max N] [--all] [--no-literal-verify] < q.g6\n";return 2;}}
 filesystem::create_directories(outdir);Stats st;string line;auto t0=chrono::steady_clock::now();
 while(getline(cin,line)&&st.parsed<maxg){++st.lines;G q;if(!parse_graph6(line,q))continue;++st.parsed;bool cub=q.n==24&&q.e.size()==36;for(int v=0;v<q.n&&cub;++v)cub&=__builtin_popcount(q.a[v])==3;if(!cub)continue;++st.cubic;if(!connected(q))continue;++st.conn;
  IntersectSolver sol(q,st);sol.solve();
  for(uint32_t S:sol.cand){++st.literal_checked;if(verify){G g=expand_marked(q,S);bool literal=(g.n==30&&g.e.size()==45&&connected(g)&&count_cycles(g,3)==3&&!has_cycle(g,4)&&!has_cycle(g,8)&&!has_cycle(g,16));if(!literal){++st.literal_mismatch;cerr<<"THEOREM_MISMATCH q="<<st.parsed<<" S="<<S<<"\n";continue;}}
