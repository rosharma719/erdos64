...
| Z5 exact coverage scan (IV.2) | `COMPUTATIONALLY_VERIFIED`, exhaustive over all `5^13-1` assignments/base | Base 1: exact 100% coverage (0 uncovered), compressed to a 48-vector certificate exactly re-verified. Bases 0,2,3: exactly 444/72/48 uncovered (corrects the previous sampled "~0" claim for bases 0,1,3). |
