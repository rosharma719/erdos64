/* Direct slot-matching generator: gap-one order-21 bridge, E=30 layer.
 * See type_b_order40_frontier.md Sections 3-4, rows G1-30-A1/A2/B1/B2/B3.
 *
 * R = B-x has 20 vertices: 18-vertex path p_0..p_17 (17 edges, p_0=a,
 * p_17=y) + two off-path vertices z1,z2 (indices 18,19). At E=30 (q=2),
 * the same five sub-cases apply (see gap-two E=30 file for the general
 * derivation), with 'ordinary' now meaning p_1..p_16,z1,z2 (18 vertices,
 * same total count as gap-two's 17 path-internal + 1 z).
 * Total 210 role configurations, looped internally.
 *
 * Non-path edges: 30-17=13.
 */
...
static void run_generate_all(void) {
    ...
    for (int v = 0; v < NV; v++) {
        ...
        else if (v == Z1_V || v == Z2_V) base_deg[v] = 0;
        else base_deg[v] = 2;
    }
