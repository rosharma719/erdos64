...
static G expand_marked(const G&q,uint32_t S){vector<int>m,c;for(int v=0;v<q.n;++v)(((S>>v)&1u)?m:c).push_back(v);if(m.size()!=4)return{};int mid[32],cid[32],port[32][32];fill(begin(mid),end(mid),-1);fill(begin(cid),end(cid),-1);memset(port,-1,sizeof(port));for(int i=0;i<4;++i)mid[m[i]]=i;for(int i=0;i<(int)c.size();++i)cid[c[i]]=12+i;for(int i=0;i<4;++i){int k=0,u=m[i];uint32_t z=q.a[u];while(z){int v=__builtin_ctz(z);z&=z-1;port[u][v]=3*i+k++;}if(k!=3)return{};}G g{30,vector<uint32_t>(30),{}};for(int i=0;i<4;++i){int b=3*i;add_edge(g,b,b+1);add_edge(g,b+1,b+2);add_edge(g,b,b+2);}for(auto [u,v]:q.e){int U=((S>>u)&1u)?port[u][v]:cid[u],V=((S>>v)&1u)?port[v][u]:cid[v];if(!add_edge(g,U,V))return{};}return g;}
...
static uint8_t allowed_bits(int L){
 // bit e says |C cap S|=e is allowed. L=3 is strengthened from {0,2} to {2}
 // because exactly four expanded triangles forbids an unmarked quotient triangle.
 static const uint8_t A[17]={0,0,0,
  (1u<<2),                     // 3
  (1u<<1),                     // 4
  (1u<<0)|(1u<<1)|(1u<<4),    // 5
  (1u<<0)|(1u<<3)|(1u<<4),    // 6
  (1u<<0)|(1u<<2)|(1u<<3)|(1u<<4), // 7
  (1u<<1)|(1u<<2)|(1u<<3),    // 8
  ...
  (1u<<0)|(1u<<1)|(1u<<4),    // 13
  (1u<<0)|(1u<<3)|(1u<<4),    // 14
  (1u<<0)|(1u<<2)|(1u<<3)|(1u<<4), // 15
  (1u<<1)|(1u<<2)|(1u<<3)|(1u<<4)  // 16
 };
 return A[L];
}
...
struct IntersectSolver{
 ...
 IntersectSolver(const G&Q,Stats&S):q(Q),st(S){
  cand.reserve(7315);for(int a=0;a<19;++a)for(int b=a+1;b<20;++b)for(int c=b+1;c<21;++c)for(int d=c+1;d<22;++d)cand.push_back((1u<<a)|(1u<<b)|(1u<<c)|(1u<<d));
 ...
 void solve(){
  // High-yield order. All lengths are eventually processed, preserving exactness.
...
int main(int argc,char**argv){string outdir=".";uint64_t maxg=ULLONG_MAX;bool all=false,verify=true;
 for(int i=1;i<argc;++i){string a=argv[i];if(a=="--outdir"&&i+1<argc)outdir=argv[++i];else if(a=="--max"&&i+1<argc)maxg=stoull(argv[++i]);else if(a=="--all")all=true;else if(a=="--no-literal-verify")verify=false;else{cerr<<"usage: t4_intersection_filter [--outdir D] [--max N] [--all] [--no-literal-verify] < q.g6\n";return 2;}}
 filesystem::create_directories(outdir);Stats st;string line;auto t0=chrono::steady_clock::now();
 while(getline(cin,line)&&st.parsed<maxg){++st.lines;G q;if(!parse_graph6(line,q))continue;++st.parsed;bool cub=q.n==22&&q.e.size()==33;for(int v=0;v<q.n&&cub;++v)cub&=__builtin_popcount(q.a[v])==3;if(!cub)continue;++st.cubic;if(!connected(q))continue;++st.conn;
  IntersectSolver sol(q,st);sol.solve();
  for(uint32_t S:sol.cand){++st.literal_checked;if(verify){G g=expand_marked(q,S);bool literal=(g.n==30&&g.e.size()==45&&connected(g)&&count_cycles(g,3)==4&&!has_cycle(g,4)&&!has_cycle(g,8)&&!has_cycle(g,16));if(!literal){++st.literal_mismatch;cerr<<"THEOREM_MISMATCH q="<<st.parsed<<" S="<<S<<"\n";continue;}}
