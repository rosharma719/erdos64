/* Direct slot-matching generator: gap-one order-21 bridge, E=29 layer.
 * See type_b_order40_frontier.md Section 4, row G1-29.
 *
 * R = B-x has 20 vertices: a spanning 18-vertex path p_0..p_17 (17 edges,
 * p_0=a, p_17=y) plus two off-path vertices z1,z2 (vertex indices 18,19).
 * At E=29 (q=0), the unique degree sequence is 2^2,3^18: a=y=2, all 16
 * path-internal vertices and both z1,z2 at exactly 3. No edge between
 * z1,z2 is excluded a priori -- it is an ordinary candidate edge.
 *
 * Non-path edges needed: 29-17=12.
 * Deficits: a:1, y:1, p_1..p_16: 1 each (16), z1:3, z2:3.
 * Total=1+1+16+3+3=24=2*12.
 */
...
static void emit_solution(void) {
    solutions_found++;
    int edges_u[12], edges_v[12], ne = 0;
...
static void gsearch(int idx, int edges_included) {
    dfs_nodes++;
    if (edges_included == 12) {
        emit_solution();
        return;
    }
    if (idx == ncand) return;
    if (edges_included + (ncand - idx) < 12) return;
...
static void run_search(void) {
    ...
    for (int i = 0; i <= 16; i++) adj[i][i + 1] = adj[i + 1][i] = 1;
